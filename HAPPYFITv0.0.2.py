...
class Funcs():
    ...
    def OnDoubleClick(self, event):
        ...
        for n in self.listaCli.selection():
            ...
            self.rg_entry.insert(END, col9)

    def add_cliente(self):
        self.variaveis()
        self.conecta_bd()

        self.cursor.execute(""" INSERT INTO clientes (nome_cliente, endereco, tel, altura, sexo, idade, funcao, rg)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", (self.nome, self.endereco, self.tel, self.altura, self.sexo, self.idade, self.funcao, self.rg))
        self.conn.commit()
        self.desconecta_bd()
        self.select_lista()
        self.limpa_cliente()
    def altera_cliente(self):
        self.variaveis()
        self.conecta_bd()
        self.cursor.execute(""" UPDATE clientes SET nome_cliente = ?, endereco = ?, tel = ?, altura = ?, sexo = ?, idade = ?, funcao = ?, rg = ?
            WHERE cod = ? """,
                            (self.nome, self.endereco, self.tel, self.altura, self.sexo, self.idade, self.funcao, self.rg, self.codigo))
        self.conn.commit()
        self.desconecta_bd()
        self.select_lista()
        self.limpa_cliente()
    ...
    def select_lista(self):
        ...
        self.conecta_bd()
        lista = self.cursor.execute(""" SELECT cod, nome_cliente, endereco, tel, altura, sexo, idade, funcao, rg FROM clientes
...
class Application(Funcs):
    ...
    def widgets_frame1(self):
        ...
        ## Criação da label RG
        self.lb_rg = Label(self.frame_1, text="RG", bg='#dfe3ee', fg='#107db2')
        self.lb_rg.place(relx=0.6, rely=0.05)
        self.lb_rg = Label(self.frame_1, text="RG de Professor", bg='#dfe3ee', fg='#107db2')
        self.lb_rg.place(relx=0.7, rely=0.15)

        self.rg_entry = Entry(self.frame_1)
        self.rg_entry.place(relx=0.6, rely=0.15, relwidth=0.1)
    ...
    def lista_frame2(self):
        ...
        self.listaCli.heading("#9", text="RG")
